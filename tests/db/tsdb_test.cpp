...
TEST(TsdbTest, BasicAssertions) {
    std::filesystem::path dbPath = std::filesystem::path("./test");
    ...
    ASSERT_EQ(ret, 0) << "Connect db failed";

    ret = createTable(engine);
    ASSERT_EQ(ret, 0) << "Create table failed";

    ret = writeDataTo(engine);
    ASSERT_EQ(ret, 0) << "Write data failed";

    ret = verifyTableData(engine);
    ASSERT_EQ(ret, 0) << "Verify data failed";

    std::cout << "PASSED data verification before we restart the db" << std::endl;

    // Restart db.
    ASSERT_EQ(engine->shutdown(), 0) << "Shutdown db failed";
    delete engine;
    ...
    // Verify data.
    ret = verifyTableData(engine);
    ASSERT_EQ(ret, 0) << "Verify data after we restart the db failed";
